package com.tcm.popularmovies.app.db;
...
public class ContentProvider extends android.content.ContentProvider {
    ...
    public String getType(Uri uri) {
        ...
        String cachedMimeType = sMimeTypeCache.get(code);
        if (cachedMimeType != null) {
            return cachedMimeType;
        }

        final Class<? extends Model> type = getModelType(uri);
        final boolean single = ((code % 2) == 0);
        ...
        sMimeTypeCache.append(code, mimeType.toString());
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        boolean single = ((URI_MATCHER.match(uri) % 2) == 0);
        final Class<? extends Model> type = getModelType(uri);
        final Cursor cursor = Cache.openDatabase().query(
                Cache.getTableName(type),
                projection,
                single ? BaseColumns._ID + "=" + getIdFromUri(uri) : selection,
                single ? null : selectionArgs,
                null,
                null,
                sortOrder);
